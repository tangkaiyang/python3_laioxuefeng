...
# handlers是URL处理模块,当handlers.py在API章节里完全编辑完再将下一行代码的双井号去掉
...
# 一下是middleware, 可以把通用的功能从每个URL处理函数中拿出来集中放到一起
# URL处理日志工厂
...
def datetime_filter(t):
    ...
    return u'%s年%s月%s日' % (dt.year, dt.month, dt.date())


async def init(loop):
    await orm.create_pool(loop=loop, **configs.db)
    # 在handlers.py完全完成后,在下面的middlewares中的list加入auth_factory
